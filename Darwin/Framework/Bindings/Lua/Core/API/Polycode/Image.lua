...
function Image:gaussianBlur(radius, deviation)
	local retVal = Polycore.Image_gaussianBlur(self.__ptr, radius, deviation)
end

function Image:createKernel(radius, deviation)
	local retVal = Polycore.Image_createKernel(self.__ptr, radius, deviation)
	if retVal == nil then return nil end
	local __c = _G["float"]("__skip_ptr__")
	__c.__ptr = retVal
	return __c
end
