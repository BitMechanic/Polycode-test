...
HelloPolycodeApp::HelloPolycodeApp(PolycodeView *view) : EventHandler() {
	...
	scene->getDefaultCamera()->lookAtEntity(player);
...
bool HelloPolycodeApp::Update() {
    ...
    Vector3 tVector = Vector3(walkSpeed * cos(playerDirection),0,walkSpeed * sin(-playerDirection));
    playerController->setWalkDirection(tVector);
	...
	if(res.collided) {
        ...
        Vector3 pCam = Vector3(7, 7, 7);
        pCam += player->getPosition();
    
        pCam.z = playerDirection;
        scene->getDefaultCamera()->setPosition(pCam);
